...
# from _neuralDA import extract_features
...
class Indicator:
    ...
    def try_restore_simple_cached(self):
        ...
        if (
            f"{self.__class__}_{meta_params[self.__class__.META_WEIGHT_INDEX]}_{meta_params[self.__class__.META_VARIANT1_INDEX]}_{meta_params[MT_WINDOW]}"
            in meta_cached
        ):
            cached = meta_cached[
                f"{self.__class__}_{meta_params[self.__class__.META_WEIGHT_INDEX]}_{meta_params[self.__class__.META_VARIANT1_INDEX]}_{meta_params[MT_WINDOW]}"
...
class RSI(Indicator):
    ...
    def prepare_variant_1_selector(self):
        return lambda: 5  # RANDOM.randrange(10, 25, 1)
...
class Strategy:
    ...
    def evaluateRSI(self, candle_sequence, rsi):
        ...
        rsi.state[(rsi.values < 40)] |= N_SOFT_BULLISH
    ...
    def evaluatePullbacks(self, candle_sequence, window):
        ...
        for i in range(5, len(candle_sequence), 1):
            ...
            closestLevelIdx = (np.abs(peaks - candle_sequence[i - 1].c)).argmin()
            closestLevelVal = peaks[closestLevelIdx]

            last_candle_size = abs(candle_sequence[i].c - candle_sequence[i].o)
            prev_to_level = abs(candle_sequence[i - 1].c - closestLevelVal)

            last_upper_wick = candle_sequence[i].h - max(
                candle_sequence[i].o, candle_sequence[i].c
            )
            last_lower_wick = (
                min(candle_sequence[i].o, candle_sequence[i].c) - candle_sequence[i].l
            )

            if last_candle_size < prev_to_level:
                continue
            ...
            if candle_sequence[i].c < candle_sequence[i - 1].c:
                if candle_sequence[i - 1].c > candle_sequence[i - 4].c:
                    if candle_sequence[i - 2].c > candle_sequence[i - 5].c:
                        # if last_lower_wick > last_upper_wick:
                        candle_sequence[i].markBearish()

            if candle_sequence[i].c > candle_sequence[i - 1].c:
                if candle_sequence[i - 1].c < candle_sequence[i - 4].c:
                    if candle_sequence[i - 2].c < candle_sequence[i - 5].c:
                        # if last_upper_wick > last_lower_wick:
                        candle_sequence[i].markBullish()

    def evaluatePullbacksNeuro(self, candle_sequence, window):
        global MODEL

        ochlv = {"O": [], "C": [], "H": [], "L": [], "V": []}

        # print("Preparing to read neural model")
        # print(len(candle_sequence))
        ...
        for i in range(0, len(candle_sequence)):
            ochlv["O"].append(candle_sequence[i].o)
            ochlv["C"].append(candle_sequence[i].c)
            ochlv["H"].append(candle_sequence[i].h)
            ochlv["L"].append(candle_sequence[i].l)
            ochlv["V"].append(candle_sequence[i].v)

        # print("ochlv extracted")
        # print(len(ochlv))

        i1 = 0

        for v2 in range(100, len(ochlv["O"]), 1):
            features = extract_features(ochlv, 0, v2, v2 - 5, v2)
            ...
            if result[0][0] > 0.9:
                candle_sequence[v2].markBullish()
            if result[0][0] < 0.1:
                candle_sequence[v2].markBearish()
    ...
    def run(self):
        ...
        # self.evaluatePullbacksNeuro(variant_candles, window)
