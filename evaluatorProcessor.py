...
class RandomMachine:
    ...
    def refresh_keys(self):
        self.primary_keys = list(
            random.randint(1, 10 ** 10) for _ in range(self.keys_depth)
        )
...
MT_HA_WEIGHT = 0
meta_option[MT_HA_WEIGHT] = lambda: RANDOM.choice([1.0, 1.25, 1.5, 1.75])
...
def generateSLTP():
    sl = RANDOM.choice([1.0, 1.5, 2.0])
    tp_dominance = RANDOM.choice([1.0, 2.0, 3.0])
    tp = sl + tp_dominance
    return sl, tp
...
MT_SLTP_REV = 12
meta_option[MT_SLTP_REV] = lambda: RANDOM.choice([False, False])
# meta_option[MT_SLTP_REV] = lambda : RANDOM.choice([False, True])
meta_params[MT_SLTP_REV] = False

#
MT_SLTP_MODE = 13
meta_option[MT_SLTP_MODE] = lambda: RANDOM.choice(["SAR", "ATR"])
meta_params[MT_SLTP_MODE] = "ATR"

#
MT_SATE_MACHINE_CONF = 14
...
MT_BAD_TRESH = 24
meta_option[MT_BAD_TRESH] = lambda: random.choice([1, 2])
meta_params[MT_BAD_TRESH] = 1

MT_HARD_SOFT_SWITCH = 25
meta_option[MT_HARD_SOFT_SWITCH] = lambda: random.choice(["HARD", "SOFT"])
meta_params[MT_HARD_SOFT_SWITCH] = "HARD"
...
def make_image_snapshot(variant_candles, variant_indicators, p1, p2):
    ...
    def drawIndicatorSegment(
        image, zone, indicator, I1, I2, minP, maxP, i1, i2, primaryColor=None
    ):
    ...
    zoneDrawables = [
        {"zone": _, "variant_candles": [], "variant_indicators": [], "min": 0, "max": 0}
        for _ in range(len(zones))
    ]

    for candle_sequence in variant_candles:
        zoneDrawables[candle_sequence.section]["variant_candles"].append(
            candle_sequence
        )
    ...
    for drawableSet in zoneDrawables:
        drawableSet["min"], drawableSet["max"] = minMaxOfZone(
            drawableSet["variant_candles"], drawableSet["variant_indicators"], p1, p2
        )
...
class CandleSequence:
    ...
    def calculate_acceptance_rate(self):
        signals_emmited = len(
            list(
                filter(
                    lambda val: val.bearish or val.bullish,
                    (_ for _ in self.variant_candles),
                )
            )
        )
        signals_accepted = len(
            list(
                filter(
                    lambda val: val.longEntry or val.shortEntry,
                    (_ for _ in self.variant_candles),
                )
            )
        )
        return (
            signals_emmited / (signals_emmited + signals_accepted)
            if (signals_emmited + signals_accepted) > 0
            else 0
        )
...
class Indicator:
    ...
    def prepare_weights_selector(self):
        return lambda: RANDOM.choice([-1.0, -0.5, 0.0, 0.5, 1, 1.5])
...
class MINUS_DI(Indicator):
    ...
    def calculate(self):
        ...
        self.values = talibMINUS_DI(
            arrHigh, arrLow, arrClose, self.resolve_variable_1()
        )
        self.calculate_dynamics()
...
class ADOSC(Indicator):
    ...
    def calculate(self):
        ...
        self.values = talibADOSC(
            arrHigh, arrLow, arrClose, arrVolume, self.fastperiod, self.slowperiod
        )
        self.calculate_dynamics()
...
class Supertrend(Indicator):
    ...
    def calculate(self):
        ...
        for index in range(ATR_PERIOD, len(self.candle_sequence.variant_candles)):
            ...
            if (
                basic_upper_band[index] != basic_upper_band[index]
                or basic_lower_band[index] != basic_lower_band[index]
            ):
            ...
            if (
                basic_upper_band[index] < final_upper_band[index - 1]
                or arrClose[index - 1] > final_upper_band[index - 1]
            ):
            ...
            if (
                basic_lower_band[index] > final_lower_band[index - 1]
                or arrClose[index - 1] < final_lower_band[index - 1]
            ):
            ...
            if (
                super_trend[index - 1] == final_upper_band[index - 1]
                and arrClose[index] <= final_upper_band[index]
            ):
                ...
                super_trend[index - 1] == final_lower_band[index - 1]
                and arrClose[index] >= final_lower_band[index]
            ):
                super_trend.append(final_lower_band[index])
...
class MACD(Indicator):
    ...
    def calculate(self):
        ...
        for index in range(0, len(self.candle_sequence.variant_candles)):
            arrClose.append(self.candle_sequence.variant_candles[index].c)
        arrClose = np.asarray(arrClose)
        macd, macdsignal, macdhist = talibMACD(
            arrClose, self.fastperiod, self.slowperiod, self.signalperiod
        )
...
def prepareRekno(variant_candles, section):
    ...
    # META PARAM
    basic_price = (
        variant_candles.h_cached.max() - variant_candles.l_cached.min()
    ) / meta_params[MT_RENKO_PREC]

    prev_o, prev_c = (
        math.floor(((variant_candles[0].h + variant_candles[0].l) / 2) / basic_price),
        math.ceil(((variant_candles[0].h + variant_candles[0].l) / 2) / basic_price),
    )
    ...
    for i, candle in enumerate(variant_candles):
        ...
        if (prev_c == lower_renko and prev_o == upper_renko) or (
            prev_o == lower_renko and prev_c == upper_renko
        ):
            o, c = prev_o, prev_c
        ...
        sequence.append(
            Candle(o, c, upper_renko, lower_renko, candle.v, sequence, candle.index)
        )
...
def prepareBoilinger(variant_candles, section, period):
    ...
    if (
        f"BOILINGER{meta_params[MT_BOILINGER_PERIOD]}_{meta_params[MT_WINDOW]}"
        in meta_cached
    ):
        return meta_cached[
            f"BOILINGER{meta_params[MT_BOILINGER_PERIOD]}_{meta_params[MT_WINDOW]}"
    ...
    for index in range(period):
        sequence.append(
            Candle(
                variant_candles.variant_candles[index].o,
...
def prepareSNR(candle_sequence, section):
    ...
    for candle in candle_sequence.variant_candles:
        ...
        sequence.append(
            Candle(mid_val, mid_val, h_val, l_val, 0, sequence, candle.index)
        )
        prev_h = h_val
        prev_l = l_val
...
def prepareVBSNR(candle_sequence, section):
    ...
    if f"VBSNR{meta_params[MT_VSNR_SENSE]}_{meta_params[MT_WINDOW]}" in meta_cached:
        return meta_cached[
            f"VBSNR{meta_params[MT_VSNR_SENSE]}_{meta_params[MT_WINDOW]}"
        ]
...
class Strategy:
    ...
    def checkHA(self, ha):
        ...
        if self.analyzeHASequence(
            greenMask, True, meta_params[MT_HK_ROW][0], meta_params[MT_HK_ROW][1]
        ):
            ha.markBullish()
        elif self.analyzeHASequence(
            greenMask, False, meta_params[MT_HK_ROW][0], meta_params[MT_HK_ROW][1]
        ):
            ha.markBearish()
    ...
    def evaluateBoilinger(self, boilingers, variant_candles, window):
        ...
        for candle in variant_candles.variant_candles[window]:
            ...
            elif candle.green and abs(candle.c - boilinger.h) > abs(
                candle.c - boilinger.o
            ):
                if not prevRed:
                    boilinger.markBullish()
                    boilinger.green = True
    ...
    def evaluateSuperTrend(self, candle_sequence, supertrend):
        ...
        supertrend.state[
            np.logical_and((close_array > supertrend.values), (supertrend.acc > 0))
        ] |= N_BULLISH
        supertrend.state[
            np.logical_and((close_array < supertrend.values), (supertrend.acc > 0))
        ] |= N_BEARISH
    ...
    def evaluateSAR(self, candle_sequence, sar):
        ...
        sar.state[
            np.logical_and((close_array > sar.values), (sar.acc > 0))
        ] |= N_BULLISH
        sar.state[
            np.logical_and((close_array < sar.values), (sar.acc > 0))
        ] |= N_BEARISH
    ...
    def evaluateMACross(self, candle_sequence, fastMa, slowMa):
        ...
        fastMa.state[
            np.logical_and((fastMa.values > slowMa.values), (fastMa.delta > 0))
        ] |= N_BULLISH
        fastMa.state[
            np.logical_and((fastMa.values < slowMa.values), (fastMa.delta < 0))
        ] |= N_BEARISH
    ...
    def evaluateDXDI(self, candle_sequence, plus_di, minus_di):
        ...
        plus_di.state[
            np.logical_and((plus_di.values > minus_di.values), (plus_di.delta > 0))
        ] |= N_BULLISH
        minus_di.state[
            np.logical_and((minus_di.values > plus_di.values), (minus_di.delta > 0))
        ] |= N_BEARISH
    ...
    def evaluateADOSC(self, candle_sequence, adosc):
        ...
        adosc.state[
            np.logical_and((adosc.state & N_BULL_ACC), (adosc.state & N_BULL_SS))
        ] |= N_BULLISH
        adosc.state[
            np.logical_and((adosc.state & N_BEAR_ACC), (adosc.state & N_BEAR_SS))
        ] |= N_BEARISH
    ...
    def checkConfluence(self, evaluated, window):
        ...
        for index in range(window.start, window.stop):
            ...
            for indicator in evaluated["variant_indicators"]:
                ...
                if meta_params[MT_HARD_SOFT_SWITCH] == "HARD":
                    ...
                    if indicator.isSoftBullish(index) and not indicator.isBullish(
                        index
                    ):
                        bullishPoints += indicator.weight

                    if indicator.isSoftBearish(index) and not indicator.isBearish(
                        index
                    ):
                        bearishPoints += indicator.weight
            ...
            if badPoints >= meta_params[MT_BAD_TRESH]:
    ...
    def run(self):
        ...
        BOILINGER = prepareBoilinger(
            variant_candles, 0, meta_params[MT_BOILINGER_PERIOD]
        )
        ...
        # evaluated["variant_indicators"] += [midpoint, hilbert, dema, tema]
        # evaluated["variant_indicators"] += [adosc]

        evaluated["variant_indicators"] += [ma200, mama, fama]
        self.evaluateMA(variant_candles, ma200)
        self.evaluateMACross(variant_candles, mama, fama)

        evaluated["variant_indicators"] += [trima, ema, wma]
        self.evaluateMA(HA, wma)
        self.evaluateMACross(HA, ema, trima)

        evaluated["variant_indicators"] += [atr, rsi]
        self.evaluateRSI(variant_candles, rsi)
        self.evaluateATR(variant_candles, atr)

        evaluated["variant_indicators"] += [volume]
        self.evaluateVolume(variant_candles, volume)

        evaluated["variant_indicators"] += [minus_di, plus_di, adx]
        self.evaluateADX(variant_candles, adx)
        self.evaluateDXDI(variant_candles, plus_di, minus_di)

        evaluated["variant_indicators"] += [macd, sar, supertrend]
...
class EVALUATOR:
    ...
    def calculateRate(self):
        ...
        profitRate = (
            self.clean_profits / (self.clean_profits + abs(self.clean_losses)) * 100
            if (self.clean_profits + abs(self.clean_losses)) > 0
            else 0
        )
        ...
        frequencyRate = (1 - (abs(self.poses - 2000) / 2000)) * 100
    ...
    def generate_image(
        ...
        p2,
        directory,
        filename_special=None,
        draw_anyway=False,
    ):
        filename = ""

        if not VISUALISE and not draw_anyway:
            return filename
        ...
        for candle in candle_sequence.variant_candles:
            if candle.isEntry():
                ...
                if candle.isLong():
                    ...
                    sltp = self.checkHitSLTP(
                        candle,
                        candle_sequence,
                        len(candle_sequence.variant_candles),
                        numPosesEx,
                    )
    ...
    def draw_forced(self, filename_postfix):
        ...
        self.image_path = self.generate_image(
            self.evaluatedTMP["target"] + self.evaluatedTMP["variant_candles"],
    ...
    def supervize_evaluated(self, evaluated):
        ...
        for signalingIndicator in evaluated["variant_indicators"]:
            ...
            create_state_record(
                sequence_name + "_VALUE", signalingIndicator.getValue(-1)
            )
            ...
            create_state_record(
                sequence_name + "_PARAMETER", signalingIndicator.variable_1
            )
    ...
    def apply_meta_window(self, O, C, H, L, V):
        return (
            O[meta_params[MT_WINDOW] :],
            C[meta_params[MT_WINDOW] :],
            H[meta_params[MT_WINDOW] :],
            L[meta_params[MT_WINDOW] :],
            V[meta_params[MT_WINDOW] :],
        )
    ...
    def evaluate(self, O, C, H, L, V):
        ...
        if isFirst:
            ...
            self.image_path = self.generate_image(
                evaluated["target"] + evaluated["variant_candles"],
                evaluated["variant_indicators"],
                lastCandle.index - 100,
                lastCandle.index,
...
class MarketProcessingPayload(Payload):
    ...
    def tryTweakMeta(self, O, C, H, L, V, tweak_major=True):
        ...
        for i in range(optimization_dim):
            rnd_meta_idx = self.get_random_meta_idx()

            meta_idxs.append(rnd_meta_idx)
            meta_backup[rnd_meta_idx] = meta_params[rnd_meta_idx]

        for otimization_idx in meta_idxs:
            test_meta_states.append([])
            for i in range(optimization_depth):
                test_meta_states[-1].append(meta_option[otimization_idx]())

        # 0:                 a,b,c,...,optimization_depth
        # 1:                 a,b,c,...,optimization_depth
        # ...:               a,b,c,...,optimization_depth
        # optimization_dim:  a,b,c,...,optimization_depth

        idx_set = [_ for _ in range(optimization_depth)]
        for index_state in product(idx_set, repeat=optimization_dim):

            # print(meta_idxs)
            # print(index_state)

            for i, index in enumerate(index_state):
                meta_index = meta_idxs[i]
                meta_params[meta_index] = test_meta_states[i][index]
            ...
            if newTR > tr:
                ...
                if tweak_major:
                    simple_log(
                        f"{round(self.last_tr,4)} -> {round(newTR,4)}", log_level=4
                    )
                    ...
                    simple_log(
                        f"{_tokenlog} |Minor| {round(self.minor_tr,4)} -> {round(newTR,4)}",
                        log_level=4,
                    )
                    self.minor_tr = newTR
        ...
        for rnd_meta_idx in meta_backup:
            meta_params[rnd_meta_idx] = meta_backup[rnd_meta_idx]
    ...
    def prepare_feedback(self):
        ...
        if not self.last_sl is None and not self.last_tp is None:
            return {
                "SL": self.last_sl,
                "TP": self.last_tp,
                "printable_metadata": SIGNALS_LOG,
            }
        else:
            return None
    ...
    def wait_for_event(self):
        ...
        while True:
            ...
            _deltalog = "{}".format(
                round(self.evaluator.clean_profits - self.evaluator.clean_losses, 3)
            ).rjust(20)

            simple_log(
                f"### TR = {_trlog}, NP = {_nplog} , DELTA = {_deltalog} /// {market_situation}",
                log_level=5,
            )
            ...
            if (
                self.last_tr < self.lower_silence_trigger
                or self.last_tr > self.higher_silence_trigger
            ):
    ...
    def areWaitingForData(self, initialTime):
        ...
        simple_log(
            f"TIME ELAPSED {elapsed_seconds} <--> NEXT CANDLE {INTERVAL_M*60}",
            log_level=2,
        )
