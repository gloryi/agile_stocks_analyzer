...
class CandleSequence:
    ...
    def maxO(self, p1, p2):
        return max(
            _.o for _ in filter(lambda _: _.index >= p1 and _.index <= p2, self.candles)
        )
    ...
    def maxH(self, p1, p2):
        return max(
            _.h for _ in filter(lambda _: _.index >= p1 and _.index <= p2, self.candles)
        )
    ...
    def maxC(self, p1, p2):
        return max(
            _.c for _ in filter(lambda _: _.index >= p1 and _.index <= p2, self.candles)
        )
    ...
    def maxL(self, p1, p2):
        return max(
            _.l for _ in filter(lambda _: _.index >= p1 and _.index <= p2, self.candles)
        )
...
class Indicator:
    ...
    def maxV(self, p1, p2):
        return max(
            _.value
            for _ in filter(lambda _: _.index >= p1 and _.index <= p2, self.values)
        )
...
class VOLUME(Indicator):
    ...
    def calculate(self):
        ...
        for index in range(0, len(self.candleSequence.candles)):
            self.values.append(
                IndicatorValue(self.candleSequence.candles[index].v, index)
            )
...
class Strategy:
    ...
    def run(self):
        evaluated = {"target": [self.candlesSequence],
                     "candles": [], "indicators": []}
...
def generateOCHLPicture(candles, indicators, p1, p2):
    ...
    def drawCandle(image, zone, candle, minP, maxP, p1, p2):
        ...
        drawLineInZone(
            img, zone, 1 - lwick, (i + 0.5) / depth, 1 -
            hwick, (i + 0.5) / depth, col
        )
        drawSquareInZone(
            img,
            zone,
    ...
    def drawIndicatorSegment(
        image, zone, v1, v2, minP, maxP, p1, p2, primaryColor=None
    ):
        i1 = v1.index - p1
        i2 = v2.index - p1
        col = (255, 255, 255)
    ...
    def drawIndicator(img, indicator, zone, minV, maxV, p1, p2):
        for v1, v2 in zip(indicator.values[:-1], indicator.values[1:]):
            drawIndicatorSegment(
                img, zone, v1, v2, minV, maxV, p1, p2, indicator.primaryColor
            )
    ...
    zoneDrawables = [
        {"zone": _, "candles": [], "indicators": [], "min": 0, "max": 0}
    ...
    for drawableSet in zoneDrawables:
        drawableSet["min"], drawableSet["max"] = minMaxOfZone(
            drawableSet["candles"], drawableSet["indicators"], p1, p2
        )
...
class MarketProcessingPayload(Payload):
    ...
    def generateStats(self):
        ...
        profitRate = self.cleanProfit / \
            (self.cleanProfit + abs(self.cleanLoses)) * 100
        ...
        for asset in self.assetsPerfomance:
            ...
            winRate = tpLocal / \
                (slLocal + tpLocal) if (slLocal + tpLocal) > 0 else 0
            ordered.append([winRate, slLocal, tpLocal,
                           asset, totalProfit, totalLoses])
        ...
        for o in ordered:
            ...
            print(
                name,
                f"SL={sl},TP={tp} | WIN_RATE = {round(wr*100,3)}% | {round(Tp,3)} / {round(Tl,3)} == {round(Tp-Tl,3)} | PROFIT_RATE == {round(pr*100,3)}% || TOTAL RATE = {round(tr*100,3)}%",
            )
    ...
    def checkHitSLTP(self, candle, candles, horizon):
        ...
        while trailingIndex < horizon:
            ...
            def within(
                sltp, trailing): return sltp >= trailing.l and sltp <= trailing.h
            ...
            if candle.isEntry():
                ...
                if candle.isLong():
                    ...
                    sltp = self.checkHitSLTP(
                        candle, candleSequence, len(
                            candleSequence.candles) - 10
                    )
                elif candle.isShort():
                    takeProfit = candle.c - atrValue * metaParam5[0]
                    stopLoss = candle.c + atrValue * metaParam5[1]
        ...
        for asset in tqdm(self.assetsList):
            ...
            if self.processBest:
                if self.token not in bestKnownMetas:
                    ...
                    m1, m2, m3, m4, m5, m6, m7, m8, m9, m10 = bestKnownMetas[
                        self.token
                    ]["meta"]
                    global metaParam1
                    global metaParam2
            ...
            self.sl += numSL
            self.tp += numTP
            self.assetsPerfomance[self.token] = [
                numSL,
                numTP,
                self.assetProfit,
                self.assetLoses,
            ...
            if self.draw:
                image_path = self.generate_image(
                    evaluated["target"] + evaluated["candles"],
...
for tweakedMeta in tweakMetaParams():
    print("\n" * 2)
    print(
        f"META {metaParam1} {metaParam2} {metaParam3} {metaParam4} {metaParam5}")
...
processor = MarketProcessingPayload(
    "dataset{timeframe}", processBest=True, draw=True)
processor.evaluate()
processor.generateStats()
