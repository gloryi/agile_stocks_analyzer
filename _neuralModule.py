...
mcp_save = ModelCheckpoint('neuralPBRose.hdf5', save_best_only=True, monitor='loss', mode='min')
reduce_lr_loss = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=7, verbose=1, min_delta=1e-4, mode='min')

estimator = KerasClassifier(model=create_baseline, epochs=100, batch_size=5, verbose=1, callbacks=[earlyStopping, mcp_save, reduce_lr_loss])
