...
metaOptions.append([0.25, 0.5, 1, 1.5, 2, 2.5, 3])
metaOptions.append(
    [[1.5, 2], [1.5, 4], [1.5, 6], [1.5, 8], [2, 4], [2, 6], [4, 4], [4, 6]]
)
metaOptions.append([0.25, 0.5, 1, 1.5, 2, 2.5, 3])
metaParams[5] = 1
metaOptions.append([0.25, 0.5, 1, 1.5, 2, 2.5, 3])
...
def generateOCHLPicture(candles, indicators, p1, p2):
    ...
    def drawCandle(image, zone, candle, minP, maxP, p1, p2):
        ...
        drawLineInZone(
            img, zone, 1 - lwick, (i + 0.5) / depth, 1 - hwick, (i + 0.5) / depth, col
        )
        drawSquareInZone(
            img,
            zone,
...
class EVALUATOR:
    ...
    def generateStats(self, lastCandle, atr):
        ...
        stats += "FROM EVALUATED {}%\n".format(
            round(bestKnownMetas["EURUSD"]["tr"] * 100, 1)
        )
            ...
            if candle.isEntry():
                ...
                if candle.isLong():
                    ...
                    sltp = self.checkHitSLTP(
                        candle, candleSequence, len(candleSequence.candles) - 10
                    )
                elif candle.isShort():
                    takeProfit = candle.c - atrValue * metaParams[4][0]
                    stopLoss = candle.c + atrValue * metaParams[4][1]
    ...
    def draw_image_ex(self, filename_postfix):
        ...
        self.image_path = self.generate_image(
            self.evaluatedTMP["target"] + self.evaluatedTMP["candles"],
...
class MarketProcessingPayload(Payload):
    ...
    def tryTweakMeta(self, O, C, H, L, V):
        ...
        virtualEvaluator = EVALUATOR(
            self.token, processBest=False, draw=False, virtual=True
        )
    ...
    def prepare_feedback(self):
        if not self.lastSL is None and not self.lastTP is None:
            return {
                "SL": self.lastSL,
                "TP": self.lastTP,
                "printable_metadata": self.last_tr,
            }
        else:
            return None
    ...
    def wait_for_event(self):
        ...
        while True:
            ...
            if not VALIDATION_MODE:
                message["text"] = self.token + " \n " + self.evaluator.generatedStats
                metaUpd = (
                    f"{round(self.priorTR*100,1)}% >>> {round(self.tweakedTR*100,1)}%"
                )
                message["text"] += "\n" + metaUpd
                message["image"] = self.evaluator.image_path
            break
