...
def generateOCHLPicture(
    ...
    def drawCandle(image, zone, candle, minP, maxP, p1, p2, v_rising=False):
        ...
        if candle.initial:
            drawLineInZone(
                img, zone, 1, (i + 0.5) / depth, 0, (i + 0.5) / depth, col, thickness=3
            )
    ...
    def drawCandles(img, candles, zone, minV, maxV, p1, p2):
        ...
        drawLineInZone(
            img, zone, 1 - oline, 0, 1 - oline, 1, (200, 0, 200), thickness=4
        )
        drawLineInZone(
            img, zone, 1 - cline, 0, 1 - cline, 1, (0, 200, 200), thickness=4
        )
...
def select_target(O, C, H, L, V, index):
    ...
    return (
        O[first_idx:last_idx],
        C[first_idx:last_idx],
        H[first_idx:last_idx],
        L[first_idx:last_idx],
        V[first_idx:last_idx],
    )
...
def soot_session(task):
    ...
    while not entry or not stop or not entry_setteled:
        ...
        cv.putText(
            img,
            active_mode + f" {WIN_STREAK} rr | {BUDGET}$",
        ...
        if input_processor.is_new_raw_coord():
            ...
            scaled_value = min_price + (1 - (horisontal_value / h)) * (
                max_price - min_price
            )
    ...
    for horizon in range(1, horizon_size, horizon_step):
        ...
        if (
            entry_activated
            and activate_stop_above
            and not profit_activated
            and not stop_activated
        ):
        ...
        img = generateOCHLPicture(
            candles[f_ind:last_ind],
            entry,
            stop,
            profit,
            entry_activated,
