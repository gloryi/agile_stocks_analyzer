...
list_of_tokens = readAssets(os.path.join(CONFIG_DIRECTORY, "capital_asset_urls.csv"))
...
p = subprocess.Popen(
    ["python3", os.path.join(project_path, "telegram_proxy.py"), "6666"]
)
...
for token in list_of_tokens:
    ...
    p = subprocess.Popen(
        [
            "python3",
            os.path.join(project_path, "evaluatorProcessor.py"),
            token,
            "MarketsPayload",
        ]
    )
