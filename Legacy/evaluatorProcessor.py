...
metaOptions = []
metaOptions.append([0.5,1,1.5,2,2.5])
metaOptions.append([0.5,1,1.5,2,2.5])
metaOptions.append([0.5,1,1.5,2,2.5])
metaOptions.append([0.5,1,1.5,2,2.5])
metaOptions.append([[1.5,1],[1.5,1.5],[1.5,2],[1.75,1],[1.75,1.5],[1.75,2],[2,1],[2,1.5],[2,2],[4,1],[4,1.5],[4,2]])
metaOptions.append([30,40,50,60,70])
metaParams[5] = 30
metaOptions.append([200,180,170,160,150])
metaParams[6] = 200
metaOptions.append([14,15,16,17,18])
metaParams[7] = 14
metaOptions.append([4,5,6,3,2])
metaParams[8] = 4
metaOptions.append([0.5,1,1.5,2,2.5])
metaParams[9] = 1
...
class Strategy():
    ...
    def checkHA(self, ha):
        ...
        if self.analyzeHASequence(greenMask, True, 2, metaParams[8]):
            ha.markBullish()
        elif self.analyzeHASequence(greenMask, False, 2, metaParams[8]):
            ha.markBearish()
    ...
    def run(self):
        ...
        ma200 = MovingAverage(metaParams[6],candles,0, (49,0,100))
        ma200.setWeight(metaParams[1])
        ma200.calculate()
        self.evaluateMA(candles, ma200, window)
        evaluated["indicators"].append(ma200)

        ma50 = MovingAverage(metaParams[5], candles,0, (49+30,0+30,100+30))
        ...
        rsi = RSI(metaParams[7],candles,3, (49,0,100))
        rsi.setWeight(metaParams[9])
        rsi.calculate()
        ...
        if candle.isLong() or candle.isShort():
            ...
            if not candle.hitTP is None:
                ...
                if candle.hitTP == candle.index:
                    drawSquareInZone(img, zone, 1-cline,(i+0.5+0.7)/depth,1-tpline,(i+0.5)/depth,(224,58,59))
                    drawSquareInZone(img, zone, 1-tpline-0.005,(i+1)/depth,1-tpline+0.005,(i-1)/depth,(224,58,59))
                else:
                    drawSquareInZone(img, zone, 1-cline,(i+0.5+0.7)/depth,1-tpline+0.005,(i+0.5)/depth,(224,58,59))
                    drawSquareInZone(img, zone, 1-tpline-0.005,(i+0.5+0.7)/depth,1-tpline+0.005,(hitInd+0.5)/depth,(224,58,59))

            if not candle.hitSL is None:
                hitInd = candle.hitSL - p1
                if candle.hitSL == candle.index:
                    drawSquareInZone(img, zone, 1-cline,(i+0.5-0.7)/depth,1-slline+0.005,(i+0.5)/depth,(25,188,224))
                    drawSquareInZone(img, zone, 1-slline-0.005,(i+1)/depth,1-slline+0.005,(i-1)/depth,(25,188,224))
                else:
                    drawSquareInZone(img, zone, 1-cline,(i+0.5-0.7)/depth,1-slline+0.005,(i+0.5)/depth,(25,188,224))
                    drawSquareInZone(img, zone, 1-slline-0.005,(i+0.5-0.7)/depth,1-slline+0.005,(hitInd+0.5)/depth,(25,188,224))
...
class EVALUATOR():
    ...
    def generateStats(self, lastCandle, atr):
        ...
        #sl, tp = lastCandle.SL, lastCandle.TP
    ...
    def fetch_market_data(self):
        ...
        ***REMOVED***
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            data = json.loads(s.recv(30000).decode("UTF-8"))
