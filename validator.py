...
def processAsset(filename=os.path.join(os.getcwd(), "ValidationDatasets", DATASET)):
    O, C, H, L, V = [], [], [], [], []
...
print("For evaluator:")
print(
    "<(str): MAJOR_BUILD> [_(str) MINOR_BUILD]?[_(str) TWEAKS_CHAIN]* <V: validation> <R: ranodm/F: fixed> [(int): port number]"
)
...
while True:
    ...
    for sliding_window_index in tqdm.tqdm(
        range(test_start, min(len(O), test_start + MAX_DEPTH))
    ):
    ...
    (
        ...
        header,
        lines,
    ) = validateFeedback(feedbackCollector, O, C, H, L)
    dump_stats(
        result, worstCase, bestCase, minus, plus, cleanLosses, cleanProfit, asset
    )
    ...
    WR = plus / (minus + plus) if (minus + plus) > 0 else 0
    PR = (
        cleanProfit / (cleanProfit + abs(cleanLosses))
        if (cleanProfit + abs(cleanLosses)) > 0
        else 0
    )
    TR = (WR + PR) / 2
